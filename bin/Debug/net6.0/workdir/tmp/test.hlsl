...
struct rmRes;
rmRes glsl_rmRes_ctor(vec3, int, bool);
...
float petal(in vec3 p, in float m )
{
	...
	p.y  -= .45;
	p.z  -= b * 1.;
	p.zy   =  glsl_mul(p.zy , rot(ouv * 2. ) );
	float pDcp = petalDcp(p.xy , w );
	p.x  = abs(p.x  );
	p.xz   =  glsl_mul(p.xz , rot(-.25 ) );
	float c1 = length(p.yz  ) - b;
	return(max(max(pDcp, abs(c1 ) - .01 ), p.z  ) );
}
vec2 repRot(in vec2 p, in float aIt )
{
	return(glsl_mul(p, rot(- (6.283185307 / aIt ) * floor((atan2(p.x , p.y  ) / 6.283185307 + .5 ) * aIt ) - 6.283185307 * .5 - 6.283185307 / (aIt * 2. ) ) ) );
}
...
{
	_pp.y  = - _pp.y ;
	_pp.xz   =  glsl_mul(_pp.xz , rot(1.016 ) ),
	_pp.xy   =  glsl_mul(_pp.xy , rot(-0.640 ) );
	float dd = 10e9,
	ee = 10e9;
	vec3 p = _pp;
	const float fsz = .25;
	...
	for(float g = 0.; g < 3.; g ++  )
	{
		p = (b = ! b) ? (p.xzy) : (p.zxy);
		float r = length(p.xy  );
		vec3 pp = vec3(log(r ) - gameTime * (.1 + ((g + 1. ) * .051 ) ), atan2(p.x , p.y  ), p.z  / r );
		float e = dot(pp.xy , n ),
		f = dot(pp.xy , vec2(n.y , - n.x  ) );
...
vec3 normal(in vec3 p, inout int m )
{
	float d = df(p, m );
	vec2 u = vec2(0., .0002 );
	return(normalize(vec3(df(p + u.yxx , m ), df(p + u.xyx , m ), df(p + u.xxy , m ) ) - d ) );
} 
struct rmRes
{
	vec3 p;
...
{
	rmRes s = glsl_rmRes_ctor(c + r * 0., 0, false );
	float d;
	int i = 0;
	for(; i < 16; i ++  )
	{
...
void mainImage(out vec4 fragColor, in vec2 fragCoord )
{
	...
	if(res.h  )
	{
		...
		color = mix(vec3(0.500, 0.763, 0.915 ), glsl_vec3(1. ), d );
...
rmRes glsl_rmRes_ctor(vec3 _p, int _i, bool _h) {
	rmRes __stru_tmp = {_p, _i, _h};
	return __stru_tmp;
}
