...
params(var(spec[in, nothing ], float, a ) )
{
	return <- (float2x2(cos(a ), sin(a ), - sin(a ), cos(a ) ) );
};
func(spec[nothing ], float, opSmoothUnion )
params(var(spec[nothing ], float, d1 ), var(spec[nothing ], float, d2 ), var(spec[nothing ], float, k ) )
{
	var(spec[nothing ], float, h ) = clamp(0.5 + 0.5 * (d2 - d1 ) / k, 0.0, 1.0 );
	return <- (lerp(d2, d1, h ) - k * h * (1.0 - h ) );
};
func(spec[nothing ], float, petalDcp )
params(var(spec[in, nothing ], float2, uv ), var(spec[in, nothing ], float, w ) )
{
	uv.x  = abs(uv.x  ) + 0.25 + 0.25 * w;
	return <- (length(uv ) - 0.5 );
};
func(spec[nothing ], float, petal )
params(var(spec[in, nothing ], float3, p ), var(spec[in, nothing ], float, m ) )
{
	...
	var(spec[nothing ], float, c1 ) = length(p.yz  ) - 0.5;
	return <- (max(max(pDcp, abs(c1 ) - 0.01 ), p.z  ) );
};
func(spec[nothing ], float2, repRot )
params(var(spec[in, nothing ], float2, p ), var(spec[in, nothing ], float, aIt ) )
{
	return <- (mul(rot(- (6.2831853070000001 / aIt ) * floor((atan2(p.x , p.y  ) / 6.2831853070000001 + 0.5 ) * aIt ) - 3.141593 - 6.2831853070000001 / (aIt * 2.0 ) ), p ) );
};
func(spec[nothing ], float, flower )
params(var(spec[in, nothing ], float3, p ), var(spec[in, nothing ], float, aIt ), var(spec[in, nothing ], float, m ) )
{
	p.xy  = repRot(p.xy , aIt );
	return <- (petal(p, m ) );
};
func(spec[nothing ], float, df )
params(var(spec[in, nothing ], float3, _pp ), var(spec[inout, nothing ], int, m ) )
{
	_pp.y  = - _pp.y ;
	_pp.xz  = mul(rot(1.016 ), _pp.xz  ),
	_pp.xy  = mul(rot(-0.64000000000000001 ), _pp.xy  );
	var(spec[nothing ], float, dd ) = 1.0E+10,
	var(spec[nothing ], float, ee ) = 1.0E+10;
	var(spec[nothing ], float3, p ) = _pp;
	var(spec[nothing ], const float, fsz ) = 0.25;
	var(spec[nothing ], const float2, n ) = float2(cos(0.3926991 ), sin(0.3926991 ) );
	var(spec[nothing ], bool, b ) = false;
	block
	{
		var(spec[nothing ], float, g ) = 0.0;
		p = (b = ! b) ? ((p.xzy)) : ((p.zxy));
		...
		var(spec[nothing ], float, e ) = dot(pp.xy , n ),
		var(spec[nothing ], float, f ) = dot(pp.xy , float2(n.y , - n.x  ) );
		block
		{
		...
		f += 1.3;
		var(spec[nothing ], float, i ) = (floor(f / l ) + 0.0 - floor(floor(f / l ) + 0.00 ) * 3.0 );
		...
		var(spec[nothing ], float, ff ) = flower(float3(e, f, pp.z  + 0.059999999999999998 ) / 0.25, smoothstep(-1.0, 1.0, r * r ) * (j ? 5.0 : 2.0 ), smoothstep(1.0, -0.0, r * r ) ) * 0.25 * r;
		ee = min(ee, ff );
		if(ee == ff )
		{
			m = j ? 1 : 0;
		...
		p = (b = ! b) ? ((p.xzy)) : ((p.zxy));
		var(spec[nothing ], float, r ) = length(p.xy  );
		var(spec[nothing ], float3, pp ) = float3(log(r ) - gameTime * 0.202, atan2(p.x , p.y  ), p.z  / r );
		var(spec[nothing ], float, e ) = dot(pp.xy , n ),
		var(spec[nothing ], float, f ) = dot(pp.xy , float2(n.y , - n.x  ) );
		block
		{
		...
		var(spec[nothing ], float, l ) = 0.65000000000000002;
		f += 1.3;
		var(spec[nothing ], float, i ) = (floor(f / l ) + 1.0 - floor(floor(f / l ) + 0.3333333 ) * 3.0 );
		f = (f - floor(f / l ) * l ) - l * 0.5;
		var(spec[nothing ], float, d ) = (length(float2(e, pp.z  ) ) - 0.014999999999999999 / r ) * r;
		var(spec[nothing ], bool, j ) = i == 0.0;
		dd = opSmoothUnion(dd, d, 0.10000000000000001 );
		...
		p = (b = ! b) ? ((p.xzy)) : ((p.zxy));
		var(spec[nothing ], float, r ) = length(p.xy  );
		var(spec[nothing ], float3, pp ) = float3(log(r ) - gameTime * 0.253, atan2(p.x , p.y  ), p.z  / r );
		var(spec[nothing ], float, e ) = dot(pp.xy , n ),
		var(spec[nothing ], float, f ) = dot(pp.xy , float2(n.y , - n.x  ) );
		block
		{
		...
		var(spec[nothing ], float, l ) = 0.65000000000000002;
		f += 1.3;
		var(spec[nothing ], float, i ) = (floor(f / l ) + 2.0 - floor(floor(f / l ) + 0.6666667 ) * 3.0 );
		f = (f - floor(f / l ) * l ) - l * 0.5;
		var(spec[nothing ], float, d ) = (length(float2(e, pp.z  ) ) - 0.014999999999999999 / r ) * r;
		var(spec[nothing ], bool, j ) = i == 0.0;
		dd = opSmoothUnion(dd, d, 0.10000000000000001 );
...
func(spec[nothing ], float3, normal )
params(var(spec[in, nothing ], float3, p ), var(spec[inout, nothing ], int, m ) )
{
...
func(spec[nothing ], rmRes, rm )
params(var(spec[in, nothing ], float3, c ), var(spec[in, nothing ], float3, r ), var(spec[inout, nothing ], int, m ) )
{
	var(spec[nothing ], rmRes, s ) = glsl_rmRes_ctor(c + r * 0.0, 0, false );
	var(spec[nothing ], float, d );
	var(spec[nothing ], int, i ) = 0;
	var(spec[nothing ], bool, _br_flag_188 ) = false;
	block
	{
		if(0 < 16 )
		{
...
params(var(spec[out, nothing ], float4, fragColor ), var(spec[in, nothing ], float2, fragCoord ) )
{
	...
	gameTime = iTime;
	var(spec[nothing ], float3, c ) = float3(0.0, 0.0, -10.0 ),
	var(spec[nothing ], float3, r ) = normalize(float3(st, 1.0 ) );
...
func(spec[nothing ], rmRes, glsl_rmRes_ctor )
params(var(spec[nothing ], float3, _p ), var(spec[nothing ], int, _i ), var(spec[nothing ], bool, _h ) )
{
	var(spec[nothing ], rmRes, __stru_tmp ) = 
	{
		_p,
		_i,
		_h 
