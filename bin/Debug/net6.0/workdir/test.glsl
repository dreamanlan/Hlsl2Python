...
float df(in vec3 _pp, inout int m) {
    //_pp.xz *= rot(gameTime), _pp.xy *= rot(gameTime);
    _pp.y = -_pp.y;
    ...
    for(float g = 0.; g < 3.; g++)
    {
        p = (b = !b) ? p.xzy : p.zxy;
        float r = length(p.xy);
        vec3 pp = vec3(log(r) - gameTime*(.1+((g+1.)*.051)), atan(p.x, p.y) /*+ cos(sqrt(r))*.2*/, p.z/r);
...
#define LIM .0002
#define MAX_IT 16
vec3 normal(in vec3 p, inout int m) { float d = df(p, m); vec2 u = vec2(0.,LIM); return normalize(vec3(df(p + u.yxx, m),df(p + u.xyx, m),df(p + u.xxy, m)) - d); }
