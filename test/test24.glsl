...
float torchFlicker()
{
	return 1.0 - 0.3 * pulse(0.0, 0.4, fract(fTime * 0.2)) * pulse(0.4, 0.5, fract(fTime * 6.0)) +
		pulse(0.1, 0.12, fract(fTime * 0.12));
}

float torchOn()
{
	return smoothstep(0.0, 0.2, fTime - 4.0);
}

float torch(vec3 cp, vec3 cv, vec3 cw, vec3 rp, float nearest, vec2 t, out vec3 torch_o, out vec3 torch_d)
{
	torch_o = cp + cv;
	torch_d = cw;
	...
	float sparkles = 1.2 + 0.7 * smoothstep(0.95, 1.0, smN2(t.xy * 50.0 + vec2(fTime * 10.0 + cos(t.y * 10.0 + fTime * 0.3) * 2.0, sin(t.x * 10.0 + fTime * 0.3) * 2.0)));
	sparkles += 0.2 * smoothstep(0.95, 1.0, smN2(t.xy * 20.0 + vec2(fTime * 10.0 + cos(t.y * 10.0 + fTime * 0.3), sin(t.x * 10.0 + fTime * 0.3))));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 cp = vec3(0.3, -0.7 + pow(0.5 + 0.5 * cos(fTime * 10.0), 0.7) * 0.01, fTime * 0.2);
	...
	vec3 ct = cp + rotateY(cos(fTime * 0.2) * 0.3 + sin(fTime * 3.0) * 0.02,
						   rotateX((0.5 + 0.5 * sin(fTime * 0.3)) * 0.2, -cl));
	...
	vec3 ro = cp, rd = rm * vec3(t2.xy * 0.3, -0.25);
	...
	l += 0.8 * vec3(1.1, 1.1, 0.8) *
		torch(cp, cv * -0.2 + cu * 0.1, rotateY(cos(fTime * 2.0) * 0.03, normalize(cw + cu * -0.2 + ttilt)), rp, nearest, t2, torch_o, torch_d);
