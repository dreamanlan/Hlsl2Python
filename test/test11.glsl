...
void fillChannel0( out vec4 fragColor, in vec2 fragCoord )
{
    gTime = float(iTime);
...
vec3 makeBloom(float lod, vec2 offset, vec2 bCoord, vec2 aPixelSize)
{
    ...
    for (int i = -3; i < 3; i++) 
    {
        for (int j = -3; j < 3; j++) 
        {
            ...
            vec3 lTextureColor = textureLod(bufferB_iChannel1, vec2(i,j) * scale + lodFactor * aPixelSize + coord, lod).rgb;
...
void fillChannel1( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 lInputColor0 = texture(bufferB_iChannel0, uv);
