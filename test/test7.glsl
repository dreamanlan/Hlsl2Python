...
bool iterateSpherePoint(inout vec3 p, inout int count, inout float orb)
{
...
vec3 chooseColor(bool found, int count, float orb)
{
    vec3 col;
    if (found)
    {
    ...
    for(int ii=0; ii<AA; ii++)
    {
        for(int jj=0; jj<AA; jj++)
        {
            ...
            if (id == 0.)
            {
                vec3 nor = pos;
                vec3 q = pos * mouRot;
                found = iterateSpherePoint(q, count, orb);
                edist = distABCD(q);
