...
void draw_obj(vec3 O, mat3 M, vec2 pos, int mode, inout float l, inout vec4 FragColor) {
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	if (true)
		draw_obj(O,M, uv, 2, l, FragColor);	
	else {
		draw_obj(O,M, 1.5*(uv+dx), 0, l, FragColor);	
		draw_obj(O,M, 1.5*(uv-dx), 1, l, FragColor);	
	}
