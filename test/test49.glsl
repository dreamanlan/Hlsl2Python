...
#define NUM_MATERIALS 4 //LIGHT_BASE_MATERIAL_ID + NUM_LIGHTS
...
{
    vec3 albedo[NUM_MATERIALS];
    int flags[NUM_MATERIALS];
};
...
Material GetMaterial(int materialID, vec3 position)
{
	...
	Material mat0 = NormalMaterial(vec3(0.6, 0.6, 0.7), 0);
    materials.albedo[CHECKER_FLOOR_MATERIAL_ID] = mat0.albedo;
    materials.flags[CHECKER_FLOOR_MATERIAL_ID] = mat0.flags;
    for(int lightIndex = 0; lightIndex < NUM_LIGHTS; lightIndex++)
    {
        Material mat1 = NormalMaterial(GetLight(lightIndex).LightColor, MATERIAL_IS_LIGHT_SOURCE);
        materials.albedo[LIGHT_BASE_MATERIAL_ID + lightIndex] = mat1.albedo;
        materials.flags[LIGHT_BASE_MATERIAL_ID + lightIndex] = mat1.flags;
    }
    
    Material mat;
    if(materialID < int(NUM_MATERIALS))
    {
        mat.albedo = materials.albedo[materialID];
        mat.flags = materials.flags[materialID];
    }
    else
    {
        // Should never get hit
        mat.albedo = materials.albedo[0];
        mat.flags = materials.flags[0];
   		return mat;
    }
